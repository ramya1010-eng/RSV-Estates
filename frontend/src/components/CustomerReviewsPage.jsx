...
const CustomerReviewsPage = ({ onNavigate }) => {
  ...
  // ── Only approved reviews shown on public page ──
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitMsg('');
    try {
      // Submit as pending — admin must approve before it shows
      const res = await axios.post(`${API}/api/customer-reviews`, {
        ...formData,
        status: 'pending',
      });
      if (res.data.success) {
  ...
  return (
    <div style={{ minHeight: '100vh', paddingBottom: '6rem' }}>
      ...
      <div style={{
        background: 'linear-gradient(180deg, #0a140d 0%, #112217 100%)',
        padding: '8rem 0 5rem', textAlign: 'center',
        position: 'relative', overflow: 'hidden',
      }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(245,130,32,0.08) 0%, transparent 70%)' }} />
        <motion.div
      ...
      <div className="container" style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '8px' }}>
            {ratingCounts.map(({ star, count }) => (
              <div key={star} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
                onClick={() => setFilterRating(filterRating === star ? 0 : star)}>
                <span style={{ fontSize: '0.75rem', opacity: 0.6, width: '12px' }}>{star}</span>
                <Star size={12} fill="#F58220" color="#F58220" />
                <div style={{ flex: 1, height: '6px', background: 'rgba(255,255,255,0.08)', borderRadius: '99px', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2.5rem', alignItems: 'start' }}>
          ...
          >
            <h4 className="serif" style={{ fontSize: '1.4rem', marginBottom: '0.4rem' }}>Share Your Experience</h4>
            <p style={{ fontSize: '0.85rem', opacity: 0.55, marginBottom: '1.5rem' }}>
              Your feedback helps others make confident decisions.
            </p>
              ...
              {showSuccess && (
                <motion.div
                  ...
                  style={{
                    ...
                    marginBottom: '1rem', fontSize: '0.85rem', color: '#2ed573',
                  }}
                >
                  <CheckCircle size={16} />
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              ...
              <div className="input-group">
                <input type="text" placeholder="Your Role (e.g. Plot Owner)" value={formData.role}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value })}
                  style={{ fontSize: '0.9rem' }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              />
              <p style={{ fontSize: '0.75rem', opacity: 0.4, margin: 0, textAlign: 'center' }}>
              ...
              <button type="submit" disabled={isSubmitting} className="submit-btn"
                style={{
                  cursor: isSubmitting ? 'not-allowed' : 'pointer',
                  opacity: isSubmitting ? 0.7 : 1, padding: '0.9rem',
